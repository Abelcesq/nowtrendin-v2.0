...
// engine and deducts 1 token. Compact pill, designed to sit in the persistent
// action bar so it's always visible (not buried at the bottom of a long list).
export function PullTrendsButton() {
  ...
  const pull = async () => {
    ...
    try {
      ...
      setMsg(d?.message ?? 'Fresh collection started.');
      setTimeout(() => {
    ...
    <View>
      ...
      >
        ...
        <Text style={{ color: '#FFFFFF', fontSize: 12, fontWeight: '800', letterSpacing: 1.2, marginLeft: 8 }}>
          {tokens <= 0 ? 'NO TOKENS LEFT' : 'PULL FRESH TRENDS'}
        </Text>
        {tokens > 0 && (
