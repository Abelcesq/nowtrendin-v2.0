...
export function TrajectoryCard({ signal, windowMs, winLabel }: { signal: Signal; windowMs?: number; winLabel?: string }) {
  ...
  return (
    <View className="bg-surface rounded-2xl border border-border p-4 mb-3" style={{ borderColor: `${col}55` }}>
      ...
      {isLoading ? (
        <ActivityIndicator color="#00C896" style={{ marginVertical: 30 }} />
      ) : pts.length < 2 ? (
          ...
          >
            {w > 0 && (
              <Svg width={w} height={H}>
                {yticks.map((t) => (
                  <G key={t}>
                    <Line x1={mL} y1={y(t)} x2={w - mR} y2={y(t)} stroke="#EEF1F4" strokeWidth={1} />
                    <SvgText x={mL - 4} y={y(t) + 3} fontSize="8" fill="#9AA3B0" textAnchor="end">{String(t)}</SvgText>
                  </G>
                ))}
                {xtickIdx.map((i) => (
                  <SvgText key={i} x={x(i)} y={H - 6} fontSize="8" fill="#9AA3B0" textAnchor="middle">{fmtAxis(pts[i].scoredAt)}</SvgText>
                ))}
                ...
                {sel != null && <Line x1={x(sel)} y1={mT} x2={x(sel)} y2={mT + plotH} stroke="#9AA3B0" strokeWidth={1} strokeDasharray="3,3" />}
              </Svg>
            )}
          </View>
