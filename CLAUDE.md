...
(future) at 1 day+ to partners. X is scanned every 12h (00:00/12:00 UTC;
founder cost ruling 2026-08-05 — pay-per-use spend had hit ~$400/mo) over the top-100
topics; deep author-gradient pulls (search/recent, ~120 posts each) fire only on movers,
max 2/day, hard-capped at X_MONTHLY_POST_BUDGET (4,800 posts/mo ≈ the $200/mo ceiling at
the observed ~$0.041/post effective rate) — status at GET /x/budget; old cadence is one
env away (X_SCAN_HOURS). Consumer/Business
...
*Last updated: 2026-09-15 — **THE OWED ENGINE DEPLOY RAN; 503 root-caused + fixed; 24h board round ruled (D1–D12); served-fabrication incident note written.** The deploy owed since `4d5aa97` ran **2026-09-14 00:46 UTC** via `.github/workflows/deploy-engine.yml` (Actions run 1: release-phase precompute wrote 600 payloads, schema `2026-08-24.1`) — rulings 2c/4c/7 are ON THE WIRE (`CHAIRMAN_RULINGS_2026-08-20D.md` is the per-item authority); 4c closes fully only after a founder re-probe of a stale topic's D block. The 09-13 web-terminal 503 was root-caused and fixed `3c12655` (builder-scoped `PG_BUILD_TIMEOUT_MS` on the two superset builds; the global 300s guard intact); the GitHub-side uptime monitor (`audits/uptime/uptime_log.csv`) reads UP since 09-14 02:08. 24h board round (`BOARD_24h-review_2026-09-15.md`): no rejects on R1–R6; option-c rejected 9-0; Chairman rulings D1–D12 implemented (`92a5ea4`·`5886140`·`dce49c8`·`30f25f9`·`e7498f6`; tier-composite prereg SEALED `e7498f6`). Challenger items executed 09-15 (founder-ordered): D9 exposure accounting **reversed from skip** → `audits/frontend/INCIDENT_served-fabrication-windows_2026-09-15.md` (intro→fix commits per class; **mobile windows stay OPEN until the Expo publish**); LIVE-claims audit of this file (this footer's "deploy STILL owed" was itself the stale claim — D7 rule now in force); pooled-n hygiene (runner emits `n_days_eff`; TEST_REPORT annotated, never rewritten); RIGHTS_REGISTER CoinAPI row marked in-house-reading/grade-C. Founder actions owed: widened CoinAPI letter · counsel (pass-through + §3.1 countersign) · **Expo publish (production mobile serves audited defects until it)** · D4/D5 rulings · PII purge-or-acceptance + fork + Drive copy.*

*Prior: 2026-09-14 — **3-week founder absence audited; deploy STILL owed but now cloud-runnable.** Full
