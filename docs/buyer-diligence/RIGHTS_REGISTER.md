...
> **Jurisdiction column (board D11 / Expansionist, 2026-09-15):** §16 gate 5 is
> jurisdiction-scoped by rule; the register now carries it in structure. A US-internal grant
> and an EU/APAC redistribution grant are different assets — the pending CoinAPI letter asks
> for the display grant jurisdiction-scoped or worldwide in writing.

| Source | Access | License basis | Jurisdiction of evidenced rights | Status |
|---|---|---|---|---|
| CoinAPI (Binance USDT-perp funding/OI) | official API, paid Tier-1 pay-per-use (~$5/mo) | **Item 9 — PARTIALLY RESOLVED 2026-09-14** against the dated snapshot `tos-snapshots/coinapi_legal_2026-09-14.txt` (Chairman-ordered). **(a) Internal/research use CLEARED:** §3.1 grants a "worldwide, non-exclusive, fully paid, non-transferable license to install and use the Software **for your own business purposes**" — the held-out collection, the sealed divergence research, and internal computation sit squarely inside that grant; §6 Restrictions bars only rent/lease/sub-license/modify of the *Services*, none of which this use does. **(b) End-user DISPLAY of the derived residual NOT YET CLEARED:** the agreement is **silent** on derived-data display/redistribution — under this register's own standard, silence is not a written grant, so display stays blocked until CoinAPI confirms in writing (draft request: `docs/buyer-diligence/COINAPI_RIGHTS_REQUEST_draft.md`) that displaying derived analytics (non-reverse-engineerable residuals) to end users is permitted. **(c) Pass-through flagged:** the Data Sources clause makes the customer "solely responsible for any use of the external Data Sources which may be subject to restrictions… and require additional license agreements with the third party" — so Binance's own market-data terms must be reviewed by counsel before display, independent of CoinAPI's answer. **(d) Provenance of (a) — annotated 2026-09-15 (Challenger R5, `BOARD_24h-review_2026-09-15.md`): the internal-use CLEARED reading is an IN-HOUSE legal reading (grade C from the Statistician's chair) — sufficient for a BLOCKING determination, insufficient for any future GRANT; counsel's engagement must countersign the §3.1 internal-use reading itself, not only the display/pass-through questions. Buyer-facing documents say display rights "not held", never "pending".** | internal use: grant is stated "worldwide" (§3.1); display: none evidenced anywhere | accruing held-out; **research use rights-clean**; display blocked on (b)+(c) |
| CoinMetrics Community | official API, keyless free tier | ⚠ **OPEN item 10**: community tiers are presumed NON-commercial until the terms say otherwise (the 403'd paid fields prove the tier); commercial-use evidence required | none evidenced (presumed non-commercial, all jurisdictions) | accruing held-out; display blocked on rights |
| Coinbase spot (premium leg) | official public API, keyless | ⚠ **OPEN item 11**: public-API market-data terms unreviewed | none evidenced (terms unreviewed) | SHELVED 2026-08-18 (noise); collector accrues |
| CFTC COT (Socrata publicreporting.cftc.gov) | official public API | US-government public record — the clean rights class (register conclusion, per gate-3 rule) | US-government public record — worldwide use of public data (counsel to confirm any per-jurisdiction database-right nuance before offshore sale) | pre-wired, disabled; the board's grade-A leg |
