...
List every skill in the repo's `.claude/skills/` (authoritative since 2026-09-14 — works on every machine and in cloud sessions; `docs/skills/` is the flat backup mirror, and the old laptop path `C:\Users\acinv\.claude\skills\` is a local copy) and confirm its SKILL.md exists.
...
| `/advisory-board` | Nine-seat adversarial board review (Chairman protocol) |
| `/scoring-assessor` | Scoring assessor over existing endpoints |
| `/engine-recovery` | Engine outage triage + safe recovery (signature table) |
| `/improve-system` | Weekly eight-lens internal audit → audits/improve-system/ |
