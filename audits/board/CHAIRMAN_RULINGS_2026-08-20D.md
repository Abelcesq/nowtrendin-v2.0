...
| 2c | Release phase **AND** schema stamp | **DONE in code** `1d3f5a6` (2026-08-24) — non-destructive transactional swap (Executioner's condition honoured first), `PAYLOAD_SCHEMA_VERSION` sealed + stamped + refused on mismatch at both serve paths, `release:` phase in Procfile (release-safe). 4/4 behaviour tests incl. the constructed deploy-version window. `[payload-rebuilt]` marker retires AT deploy. **DEPLOYED 2026-09-14 00:46 UTC** (GitHub Actions run 1: release phase ran, `serve_payloads written: 600 (schema 2026-08-24.1)`, swap transactional) |
...
| 4c | NULL tri-state: serve fix **AND** backfill | **DONE in code** `4d5aa97` + `e1336be` (all four serve paths; NULL serves UNKNOWN as a distinct fact — the tri-state supersedes a value backfill, which round 5 established cannot be honest for pre-epoch rows). **DEPLOYED 2026-09-14 00:46 UTC** (Actions run 1; release precompute wrote 600 payloads, schema 2026-08-24.1). Re-probe of a stale topic's D block on the wire still owed (founder browser or next session) — DONE only after it |
